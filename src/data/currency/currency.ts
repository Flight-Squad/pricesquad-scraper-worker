export function parseLocaleNumber(stringNumber: string, locale: string, options?: any) {
    // https://stackoverflow.com/a/42213804
    const numFormatter = makeNumFormatter(locale, options);
    const thousandSeparator = numFormatter.format(1111).replace(/1/g, '');
    const decimalSeparator = numFormatter.format(1.1).replace(/1/g, '');
