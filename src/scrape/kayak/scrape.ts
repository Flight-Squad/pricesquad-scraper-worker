...
export async function kayakTripData(html) {
    ...
    scraper('div[class*="Flights-Results-FlightResultItem"]').each(function(i, elem) {
        ...
        scraper(this)
            .find('.resultInner')
            .find('.section.duration')
            .find('.top')
            .each(function(i, elem) {
                let duration = scraper(this)
                    .text()
                    .trim();
...
function makeAggregatorTripsData(prices, stops, airlines, durations) {
    const dataIsConsistent =
        prices.length === stops.length && stops.length === durations.length && durations.length === airlines.length;
    const trips = [];
    if (dataIsConsistent) {
